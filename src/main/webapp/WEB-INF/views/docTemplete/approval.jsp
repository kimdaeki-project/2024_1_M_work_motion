	...
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination">
				<c:if test="${!pager.start}">
					<li class="page-item"><a class="page-link start_referrer" data-start="${pager.startNum-1}"
							data-search="${pager.search}" data-kind="${pager.kind}" aria-label="Previous"> <span
								class="start_referrer" data-start="${pager.startNum-1}"
								aria-hidden="true">&laquo;</span>
						</a></li>
				</c:if>

				<c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
					<li class="page-item"><a class="page-link referrer" data-page="${i}" data-search="${pager.search}"
							data-kind="${pager.kind}">${i}</a></li>
				</c:forEach>

				<c:if test="${!pager.last}">
					<li class="page-item"><a class="page-link last_referrer" data-last="${pager.lastNum+1}"
							data-search="${pager.search}" data-kind="${pager.kind}" aria-label="Next">
		...
		<div>
			<form class="row g-3" id="referrer_search">
				<div class="col-auto">
					<select name=kind id="ref_kind" class="form-select" aria-label="Default select example">
